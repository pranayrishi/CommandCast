...
class LMMEngineOpenAI(LMMEngine):
    def __init__(
        ...
        **kwargs,
        ...
        assert temperature == 1, "OpenAI models require temperature to be set to 1"
        self.model = model
        self.base_url = base_url
        self.api_key = api_key
        self.organization = str(organization) if organization is not None else None
        self.request_interval = 0 if rate_limit == -1 else 60.0 / rate_limit
        self.llm_client = None
        self.temperature = 1  # Can force temperature to be the same (in the case of o3 requiring temperature to be 1)
